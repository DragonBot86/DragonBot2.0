	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `¡¡𝗛𝗼𝗹𝗮 𝗡𝗲𝗳𝗮𝘀𝘁𝗼!! @${num.split('@')[0]}🐉\n\n  『${mdata.subject}』\n\n《💀》𝑷𝒓𝒆𝒔𝒆𝒏𝒕𝒂𝒓𝒔𝒆 𝒄𝒐𝒏:
𝙁𝙤𝙩𝙤:
𝙉𝙤𝙢𝙗𝙧𝙚:
𝙀𝙙𝙖𝙙:
𝙋𝙖𝙞𝙨:《💀》\n\n┉┅━ 𝗥𝗲𝗰𝘂𝗲𝗿𝗱𝗮 𝗹𝗲𝗲𝗿 𝗹𝗮𝘀 𝗿𝗲𝗴𝗹𝗮𝘀,𝗼𝗸?
\n\n『 𝖤𝗏𝗂𝗍𝖺 𝗁𝖺𝖼𝖾𝗋 𝗆𝖺𝗆𝖺𝖽𝖺𝗌, 𝗆𝖾𝗃𝗈𝗋 𝖽𝖺𝗆𝖾𝗅𝖺𝗌:)』`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Bueno, se fue @${num.split('@')[0]} 👋\n\nAdios pedazo de aborto`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	case 'x':
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _by Dragón_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
